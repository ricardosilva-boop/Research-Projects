...
### How to Execute all Cells Quickly

1. **Environment Setup:** Ensure you have Python 3.8+ installed. The code is optimized for a Jupyter Notebook environment (`.ipynb`), but can also be run as a standard Python script (`.py`).
2. **Prepare the Data:** Place your input dataset named `Cu_Mo_processed.csv` in the same directory as the script. The dataset must contain spatial coordinates (X, Y, Z) and log-transformed grades (Mo, Cu).
3. **Run the Code:**
...
4. **Check Outputs:** The script will output a block model file (`modelo_blocos_deswik.csv`) and save four high-resolution PDF plots in your working directory.
...
* **Cell 2: Variogram Parameters & Spatial Functions**
  Defines the spatial continuity of the deposit. It sets up the rotation matrix based on the azimuth () and defines the primary (Spherical) and secondary (Gaussian) covariance models, alongside nugget effects, ranges, and the correlation coefficient ().
...
* **Cell 4: 3D Grid Generation & Convex Hull Masking**
  Generates a 3D grid with blocks covering the spatial extent of the data. To avoid estimating blocks in empty space (extrapolation), it uses a Delaunay triangulation and Convex Hull to mask and keep only the blocks that fall within the geometric limits of the actual drill holes.

* **Cell 5: Block Estimation & CSV Export**
  Iterates through all valid blocks (using a `tqdm` progress bar) and applies the Cokriging function. It applies a log-normal back-transformation formula to convert the estimates back to original units. Finally, it exports the results to `modelo_blocos_deswik.csv`.
...
* **Cell 8: Advanced Statistical Metrics**
  Calculates the Coefficient of Determination () to assess how well the block model explains the variance of the real data, alongside Skewness and Kurtosis to evaluate the shape and asymmetry of the error distributions.
...
* **Cell 10: Real vs. Estimated Scatterplots**
  Plots the final scatterplots comparing the real grades against the estimated grades. It includes a red reference dashed line to visually evaluate overestimation or underestimation trends. These plots are also exported as PDFs.
